...
import type {BoundingBoxType} from '../../types/bounds';
 ...
 */
function inBounds(point: FeatureType, bounds: BoundingBoxType): boolean {
 ...
 */
function getTrackBounds(track: TrackType): Object {
...
export default class PointGenerator {
   ...
   */
  setTracks(tracks: Array<TrackType>): PointGenerator {
    this.tracks = tracks;
   ...
   */
  getIdsInBounds(bounds: BoundingBoxType): Set<string> {
    const [sw, ne] = bounds;
    const [minX, minY, maxX, maxY] = [sw[0], sw[1], ne[0], ne[1]];

    return new Set(
      this.tree
        .search({minX, minY, maxX, maxY})
        .map((item: Object): string => item.id)
    );
  }

  /**
   * Generates points form the tracks given the current map bounds.
   * @param {BoundingBoxType} currentBounds  The bounding box
   * @returns {Array<FeatureType>}  The points
   */
  getPoints(currentBounds: BoundingBoxType): Array<FeatureType> {
    ...
    const points = [];
    this.points.forEach((point: FeatureType): number => points.push(point));
